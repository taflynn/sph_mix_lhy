...
from numpy import pi, sqrt, tan, cos
...
def Ft(t,z,x):
    ...
    """
    pet_eq = cos(t)**(-2)*(tan(t)**2*sqrt(0.5*(tan(t)**2*(1+x/z) + 0.25*tan(t)**4*(1+z**(-2))) + sqrt(0.25*((tan(t)**2 + 0.25*tan(t)**4)-((x/z)*tan(t)**2 + 0.25*z**(-2)*tan(t)**4))**2 + (x/z)*tan(t)**4)) + tan(t)**2*sqrt(0.5*(tan(t)**2*(1+x/z) + 0.25*tan(t)**4*(1+z**(-2))) - sqrt(0.25*((tan(t)**2 + 0.25*tan(t)**4)-((x/z)*tan(t)**2 + 0.25*z**(-2)*tan(t)**4))**2 + (x/z)*tan(t)**4))-((1+z)/(2*z))*tan(t)**4 - (1+x)*tan(t)**2 + (1/(1+z))*((1+x*z)**2 + z*(1+x)**2))
    return pet_eq
...
def eq_dens_ulck(m1, m2, a11, a22, a12):
    ...
    """
    if m1 == m2:
        n01_lck = (25*pi/1024)*(a12 + sqrt(a11*a22))**2/(a11**1.5*a22*(sqrt(a11) + sqrt(a22))**5)
        n02_lck = (25*pi/1024)*(a12 + sqrt(a11*a22))**2/(a22**1.5*a11*(sqrt(a11) + sqrt(a22))**5)
...
def fun(x, a11, a22, a12):
    ...
    return [(((a11*x[0] + a12*x[1])-((0.5*a11*x[0]**2 + a12*x[0]*x[1] + 0.5*a22*x[1]**2)/(x[0] + x[1]))) \
            +((64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**1.5*(2.5*a11 - (a11*x[0] + a22*x[1])/(x[0] + x[1])))), \
            (((a22*x[1] + a12*x[0])-((0.5*a11*x[0]**2 + a12*x[0]*x[1] + 0.5*a22*x[1]**2)/(x[0] + x[1]))) \
            +((64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**1.5*(2.5*a22 - (a11*x[0] + a22*x[1])/(x[0] + x[1]))))]
...
def jac(x, a11, a22, a12):
    ...
    """
    return [[(x[0] + x[1])**(-2)*(0.5*a11*x[0]**2 + a11*x[0]*x[1] + (a11 -a12 + 0.5*a22)*x[1]**2 + (64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**0.5*(1.5*a11*(x[0] + x[1])*(1.5*a11*x[0] + (2.5*a11 - a22)*x[1]) + (a11*x[0] + a22*x[1])*(a22*x[1] - a11*x[1]))),\
            (x[0] + x[1])**(-2)*(0.5*a11*x[0]**2 + (2*a12 - a22)*x[0]*x[1] - 0.5*a22*x[1]**2 + (64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**0.5*(1.5*a22*(x[0] + x[1])*(1.5*a11*x[0] + (2.5*a11 - a22)*x[1]) + (a11*x[0] + a22*x[1])*(a11*x[0] - a22*x[0])))], \
            [(x[0] + x[1])**(-2)*(0.5*a22*x[1]**2 + (2*a12 - a11)*x[0]*x[1] - 0.5*a11*x[0]**2 + (64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**0.5*(1.5*a11*(x[0] + x[1])*((2.5*a22 - a11)*x[0] + 1.5*a22*x[1]) + (a11*x[0] + a22*x[1])*(a22*x[1] - a11*x[1]))), \
            (x[0] + x[1])**(-2)*(0.5*a22*x[1]**2 + a22*x[0]*x[1] + (a22 -a12 + 0.5*a11)*x[0]**2 + (64/(15*sqrt(pi)))*(a11*x[0] + a22*x[1])**0.5*(1.5*a22*(x[0] + x[1])*((2.5*a22 - a11)*x[0] + 1.5*a22*x[1]) + (a11*x[0] + a22*x[1])*(a22*x[0] - a11*x[0])))]]
