...
def energy_eqm_dens_lck(phi,V,r,dr,N_lck):
    ...
    E_ke  = (1/N_lck)*4*pi*np.trapz(r**2*N_lck*0.5*np.abs(dphi_dr)**2)*dr 
    E_pot = (1/N_lck)*4*pi*np.trapz(r**2*N_lck*V*np.abs(phi)**2)*dr
    E_int = (1/N_lck)*4*pi*np.trapz(r**2*N_lck**2*(-1.5*np.abs(phi)**4))*dr
    E_lhy = (1/N_lck)*4*pi*np.trapz(r**2*N_lck**2.5*np.abs(phi)**5)*dr
...
def energy_uneqm_dens_ulck(psi1,psi2,r,V1,V2,dr,gam1,gam2,alpha,beta,eta,N1,N2,z):
    """
    This function calculates the separate energy components of the density-unlocked mixture (w/ LHY) for
    both components of the mixture. Thus for the i-th component we have the outputs:
    (1) Ei_ke - Kinetic energy 
    ...
    E_ke = E_coef*4*pi*np.trapz(r**2*(0.5*N1*gam1*np.abs(dpsi1_dr)**2 + 0.5*N2*gam2*beta**(-1)*np.abs(dpsi2_dr)**2))*dr
    E_pot = E_coef*4*pi*np.trapz(r**2*(N1*V1*np.abs(psi1)**2 + N2*beta**(-1)*V2*np.abs(psi2)**2))*dr
    E_int = E_coef*4*pi*np.trapz(r**2*(0.5*N1**2*np.abs(psi1)**4 + 0.5*N2**2*np.abs(psi2)**4 + N1*N2*eta*np.abs(psi1)**2*np.abs(psi2)**2))*dr
    E_lhy = E_coef*4*pi*np.trapz(r**2*(N1**2.5*alpha*(1 + z**0.6*((N2*np.abs(psi2)**2)/(N1*np.abs(psi1)**2)))**2.5*np.abs(psi1)**5))*dr
