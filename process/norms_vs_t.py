...
def load_in_cal_norms(dirarg, sim_num):
    ...
    for i in range(0, no_of_frames):
        ...
        R_1 = r[(absolute(psi1_saved[:, i])**2.0 
                 > 0.05*(absolute(psi1_saved[:, i])**2.0).max())][-1]
        R_2 = r[(absolute(psi2_saved[:, i])**2.0 
                 > 0.05*(absolute(psi2_saved[:, i])**2.0).max())][-1]
...
def run_ulck_process(dirarg, sim_num):

    [psi1_cat, psi2_cat, norms_1, norms_2, deln_r0, times, theory_omg1, theory_omg2] = load_in_cal_norms(dirarg, sim_num)
    ...
    savetxt('../data/' + dirarg + '/norms1_vs_t.csv', norms1_vs_t, delimiter=',', fmt='%18.16f')
    savetxt('../data/' + dirarg + '/norms2_vs_t.csv', norms1_vs_t, delimiter=',', fmt='%18.16f')
    savetxt('../data/' + dirarg + '/deln_r0_vs_t.csv', deln_r0_vs_t, delimiter=',', fmt='%18.16f')
...
if __name__ == '__main__':
    ...
    run_ulck_process(args.READ_PATH[0], args.SIM_NUM[0])
